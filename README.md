...
기준일은 **2026-09-04**다. 모델 준비와 1차 정적평가를 마쳤고, 노트북에서
성공 후보 21개의 성능 우선 지연시간 재측정과 3축 Pareto 산출까지 완료했다.
...
| 2차 TensorRT 평가 | 통과 22개 | **완료** | 21개 완료, Q03 build 실패; performance 통제 지연시간 63/63 완료 |
| 3차 Pareto 분석 | 2차 성공 후보 | **완료** | 3축 Pareto 및 30 FPS 배포 후보 C01·R01 |
...
최종 실측에서 C01은 B01 대비 중앙 지연시간을 44.79%, engine 크기를 49.95%
...
세 반복 median의 CV가 5%를 초과하면 동일 엔진의 지연시간 3회를 한 번만 전체
재측정한다. 재측정이 수행되면 두 번째 라운드를 공식 지연시간으로 사용하고 첫
라운드도 원시 근거로 보존한다. 두 번째 CV도 5%를 초과하면 Pareto 값은 보존하되
최종 권장 모델에서는 제외한다. 엔진과 437장 정확도 평가는 다시 만들지 않는다.
공식 비교는 AC 전원과 `performance` 전원 정책을 고정한 상태에서 엔진 생성 성공
후보 21개 전체를 대상으로 수행한다.
첫 반복 전 30초 동안 화면 갱신과 부하가 정착하도록 기다린다. 각 반복 직전 CPU
사용률 30% 이하, GPU 사용률 50% 이하, GPU memory-controller 사용률 50% 이하,
GPU 온도 75°C 이하인 상태가 1초 간격 3개 표본에서 유지되어야 한다. 표본 간 CPU
사용률 범위는 20%p, GPU와 GPU memory-controller 범위는 각각 50%p, GPU 온도 범위는
6°C 이하여야 한다. 첫 안정 구간 평균과 이후 구간 차이는 CPU 20%p, GPU와
memory-controller는 각 50%p, GPU 온도는 15°C 이내로 제한한다. 순간적인 화면·센서 변동은 허용하면서
지속적인 고부하 상태에서 측정을 시작하는 것은 막기 위한 기준이다.
