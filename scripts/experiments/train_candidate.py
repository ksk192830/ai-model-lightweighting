...
def main() -> int:
    ...
    if experiment["method"] != "nvidia-2to4":
        raise ValueError("Recovery training currently supports NVIDIA 2:4 only.")
    ...
    profile = registry.defaults["fine_tuning"]["recovery_2to4"]
    hardware = resolve_hardware(args.device)
    output_dir = repository_path(
        args.output_dir or Path(profile["output_dir"])
    )
    ...
    mask_callback = make_lightning_mask_callback(controller)
    trainer_kwargs = {
        "accelerator": hardware["accelerator"],
        "devices": hardware["devices"],
    }
    ...
    trainer.callbacks.append(mask_callback)
    ...
    run_report = {
        **runtime_metadata(),
        ...
        "mask_layers": len(controller.masks),
    }
