...
- `evaluate_coco_tensorrt.py`: COCO bbox/mask AP·AP50·AP75·크기별 AP·AR100,
  클래스별 AP/AR 및 mask mIoU/클래스별 IoU 측정
- `benchmark_baseline.py`: PyTorch checkpoint 또는 TensorRT engine 벤치마크
- `visualize_inference_stream.py`: 후보 예측의 시각 비교

모든 engine은 동일한 test 437장, batch 1, confidence와 IoU 기준을 사용한다.
latency는 동일 32장에서 반복마다 warm-up 20회 후 200회를 3회 수행하며 pooled
median/P95/P99와 반복 median의 95% 구간을 남긴다. 432 입력 실험은 504 입력
후보와 분리해 보고한다.
