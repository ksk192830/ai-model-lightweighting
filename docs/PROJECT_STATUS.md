...
12. 공식 결과를 데스크톱에 동기화하고 26개 전체 최종 판정표, 후보 흐름,
    정확도–지연시간·정확도–engine 크기와 CV 그림, 논문 근거 연결표를 생성했다.
13. 최종 추천은 임의 가중합 단일 우승자가 아니라 용도별로 고정했다. C01은
    균형·Mask AP·engine 크기 우선, R01은 지연시간 우선 후보로 사용한다.
...
- Stage 3 논문 개조식 구성안과 줄글 초안을 최종 수치로 갱신하고 Notion에 반영
- 참고문헌의 저자·학회·페이지·DOI를 제출 양식에 맞게 검증
- 저장소에 근거가 없는 데이터 수집·라벨링 정보, 기존 ASK 논문 정보와 사사 문구 확보
...
논문 작성용 확정 입력은 [Stage 3 최종 분석](reports/stage3-final-analysis.md)과
[근거 연결표](paper/stage3-evidence-map.md)를 우선 사용한다.
