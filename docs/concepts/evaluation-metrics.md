...
즉시 탈락시키지 않고 동일한 부하 안정화 조건에서 지연시간 3회를 한 번 전체
재측정한다. 첫 라운드는 폐기하지 않고 근거로 보존하며 두 번째 라운드를 공식값으로
사용한다. 두 번째 CV도 5%를 넘으면 해당 수치를 Pareto 분석에는 보존하되 측정
안정성이 충분하지 않은 모델로 표시하고 최종 권장 후보에서는 제외한다. 엔진과
정확도 결과는 재사용한다.
...
30 FPS를 보장한다는 뜻은 아니므로 두 값은 별도로 보고한다. P95가 33.33 ms를
초과하면 tail-latency 경고를 기록하되, 이를 hard gate나 Pareto 제외 조건으로
사용하지 않는다.
