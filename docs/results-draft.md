...
| Ultralytics PT† | 512 | 0.0107 | 0.0095 | 0.0084 | 0.0017 | 36.87 | 27.58 | 29.65 | 5.70 |

† 기존 Ultralytics PyTorch 모델. 현재 테스트셋에서 정확도가 비정상적으로
낮아 원인 규명 전까지 속도 참고용으로만 사용한다(4.4절). R01(432)과
Ultralytics(512)는 입력 크기가 504×504 후보와 다르므로 직접 비교에 주의한다.
...
넷째, 기존 Ultralytics 모델(parking_front.pt)은 36.87 FPS, 5.70 MB로
속도·크기 면에서는 가장 우수했으나 현재 테스트셋에서 mAP50-95 0.0017이라는
비정상적으로 낮은 정확도를 보였다. 클래스 정의는 일치하므로 전처리 또는
라벨 좌표계 불일치가 의심되며, 원인 규명 전까지 이 모델의 수치는 속도
참고용으로만 제시하고 정확도 비교와 Pareto 분석에서는 제외했다. 이 원인
분석은 후속 작업으로 남긴다.
